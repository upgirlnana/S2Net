...
class BodyBasicBlock(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, stride=1, downsample=None):
        super(BodyBasicBlock, self).__init__()
...
class BodyBottleneck(nn.Module):
    expansion = 4

    def __init__(self, inplanes, planes, stride=1, downsample=None):
        super(BodyBottleneck, self).__init__()
...
class BodyResNet(nn.Module):
    def __init__(self, last_stride=2, block=BodyBottleneck, layers=[3, 4, 6, 3]):
        self.inplanes = 64
        super().__init__()
        ...
        self.layer4_channel = nn.Sequential(nn.Linear(2048, int(2048 / 8)),
                                            ...
                                            nn.Sigmoid())
        # self.head_spatial = nn.Sequential(nn.Conv2d(2048, 2048, kernel_size=3, padding=1),
        #                                   # nn.AdaptiveAvgPool2d((16, 8)),
        #                                   nn.ReLU(inplace=True),
        ...
        #                                   nn.Sigmoid())
        # self.upper_spatial = nn.Sequential(nn.Conv2d(2048, 2048, kernel_size=3, padding=1),
        #                                    # nn.AdaptiveAvgPool2d((16, 8)),
        #                                    nn.ReLU(inplace=True),
        #                                    nn.Conv2d(2048, 2048, kernel_size=3, padding=1),
        #                                    nn.ReLU(inplace=True),
        #                                    nn.Conv2d(2048, 1, kernel_size=1),
        #                                    nn.Sigmoid())
        # self.lower_spatial = nn.Sequential(nn.Conv2d(2048, 2048, kernel_size=3, padding=1),
        #                                    # nn.AdaptiveAvgPool2d((16, 8)),
        #                                    nn.ReLU(inplace=True),
        #                                    nn.Conv2d(2048, 2048, kernel_size=3, padding=1),
        #                                    nn.ReLU(inplace=True),
        #                                    nn.Conv2d(2048, 1, kernel_size=1),
        #                                    nn.Sigmoid())
        # self.shoes_spatial = nn.Sequential(nn.Conv2d(2048, 2048, kernel_size=3, padding=1),
        #                                    # nn.AdaptiveAvgPool2d((16, 8)),
        #                                    nn.ReLU(inplace=True),
        #                                    nn.Conv2d(2048, 2048, kernel_size=3, padding=1),
        #                                    nn.ReLU(inplace=True),
        #                                    nn.Conv2d(2048, 1, kernel_size=1),
        #                                    nn.Sigmoid())
        self.head_spatial = nn.Sequential(nn.Conv2d(2048, 1024, kernel_size=3, padding=1),
                                           # nn.AdaptiveAvgPool2d((16, 8)),
                                           nn.ReLU(inplace=True),
                                           ...
                                           nn.Sigmoid())
        self.upper_spatial = nn.Sequential(nn.Conv2d(2048, 1024, kernel_size=3, padding=1),
                                           # nn.AdaptiveAvgPool2d((16, 8)),
                                           nn.ReLU(inplace=True),
                                           nn.Conv2d(1024, 512, kernel_size=3, padding=1),
                                           nn.ReLU(inplace=True),
                                           nn.Conv2d(512, 1, kernel_size=1),
                                           nn.Sigmoid())
        self.lower_spatial = nn.Sequential(nn.Conv2d(2048, 1024, kernel_size=3, padding=1),
                                           # nn.AdaptiveAvgPool2d((16, 8)),
                                           nn.ReLU(inplace=True),
                                           nn.Conv2d(1024, 512, kernel_size=3, padding=1),
                                           nn.ReLU(inplace=True),
                                           nn.Conv2d(512, 1, kernel_size=1),
                                           nn.Sigmoid())
        self.shoes_spatial = nn.Sequential(nn.Conv2d(2048, 1024, kernel_size=3, padding=1),
                                           # nn.AdaptiveAvgPool2d((16, 8)),
                                           nn.ReLU(inplace=True),
                                           nn.Conv2d(1024, 512, kernel_size=3, padding=1),
                                           nn.ReLU(inplace=True),
    ...
    def forward(self, x):
        ...
        # import pdb
        # pdb.set_trace()
        head_mask = self.head_spatial(x)
        upper_mask = self.upper_spatial(x)
        lower_mask = self.lower_spatial(x)
        shoes_mask=self.shoes_spatial(x)
        x = self.conv(x)
        head=x*head_mask
        upper=x*upper_mask
        lower=x*lower_mask
        ...
        return [x,head,upper,lower,shoes],[x_att1_spatial,head_mask,upper_mask,lower_mask,shoes_mask]
